...
function parseComponents(buildFile) {
    ...
    let componentDec = /(.*?)Component\s=\s__decorate\(([\s\S]*?)\["Component"\]\)\((\{[\s\S]*?\})\),/g
    let componentMatch;
    while ((componentMatch = componentDec.exec(data)) !== null) {
        ...
        try {
            ...
            evalC.cName = componentMatch[1].trim() + "Component"
            components.push(evalC)
            if (evalC.cName == "AppModule") {
                console.log(componentMatch[0])
                console.log(evalC)
